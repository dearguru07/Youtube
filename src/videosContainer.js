...
const VideosContainer=()=>{
  ...
  async function youtube(){
    const data=await fetch("https://youtube.googleapis.com/youtube/v3/videos?part=snippet%2CcontentDetails%2Cstatistics&chart=mostPopular&maxResults=50&regionCode=IN&key=AIzaSyBYih4RX3Cf7H617owkk2KJxxdbfX0_pD4")
  ...
  if(videoArray.length==0){
    ...
    {
  videoArray.map((x)=>{
   return <Link to={"/videos/"+x.id} className="lines"><Video data={x}/></Link>
  })
}
  </div>
  }
